...
class FileAndMicAudioDevice(private val context: Context) : AudioDevice {
    ...
    */
    private val fileCapturerRunnable = object : Runnable {
        override fun run() {
            ...
            var bytesRead: Int = 0
            try {
                if (dataInputStream != null && dataInputStream!!.read(
            ...
            capturerHandler!!.postDelayed(
                this,
                CALLBACK_BUFFER_SIZE_MS.toLong()
            )
        }
    }
    ...
    */
    private val speakerRendererRunnable = object : Runnable {
        override fun run() {
            Process.setThreadPriority(Process.THREAD_PRIORITY_URGENT_AUDIO)
            try {
                audioTrack!!.play()
            ...
            while (keepAliveRendererRunnable) {
                ...
                AudioDevice.audioDeviceReadRenderData(
                    renderingAudioDeviceContext!!,
                    readByteBuffer!!
                )
                var bytesWritten = 0
  ...
  */
    ...
    override fun onInitCapturer(): Boolean {
        ...
        val bufferSizeInBytes =
            Math.max(BUFFER_SIZE_FACTOR * minBufferSize, micWriteBuffer!!.capacity())
